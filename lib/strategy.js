    ...
    if (clientData.type === 'webauthn.get') {
      ...
      try {
        if (self._passReqToCallback) {
          ...
          switch (arity) {
          case 9:
            return self._register(req, ctx.user, credentialId, pem, flags, authenticatorData.signCount, response.transports, vAttestation, registered);
          case 8:
            return self._register(req, ctx.user, credentialId, pem, flags, authenticatorData.signCount, response.transports, registered);
          case 7:
            return self._register(req, ctx.user, credentialId, pem, flags, authenticatorData.signCount, registered);
          case 6:
            return self._register(req, ctx.user, credentialId, pem, flags, registered);
          default:
            return self._register(req, ctx.user, credentialId, pem, registered);
          }
        } else {
          var arity = self._register.length;
          switch (arity) {
