...
exports.verify = function(attStmt, authData, hash) {
  ...
  var attCert = att.cert[0];
  if (attCert.publicKey.asymmetricKeyType != 'ec') {
    throw new TypeError('FIDO U2F certificate public key must be an EC key');
  }
  if (attCert.publicKey.asymmetricKeyDetails.namedCurve != 'prime256v1') {
    throw new TypeError('FIDO U2F certificate public key must be an EC key over P-256 curve');
  }
  ...
  if (x.length != 32) {
    throw new Error('FIDO U2F credential public key must have 32-byte X coordinate');
  }
  
  var y = data.attestedCredentialData.credentialPublicKey.get(-3);
  if (y.length != 32) {
    throw new Error('FIDO U2F credential public key must have 32-byte Y coordinate');
  }
